...
RETARDO_MS = 20  # velocidad de animación mas rapida para 100 datos
...
# Algoritmo elaborado en clase: Bubble Sort
def bubble_sort_steps(data, draw_callback):
    n = len(data)
    for i in range(n - 1):
        for j in range(0, n - i - 1):
            draw_callback(activos=[j, j + 1]); yield #actualiza el dibujo en cada comparación
            
            if data[j] > data[j + 1]:
                data[j], data[j + 1] = data[j + 1], data[j]
    draw_callback(activos=[])#actualiza el dibujo despues de su ultima iteracion y señala que ha terminado
...
def dibujar_barras(canvas, datos, activos=None):
    ...
    for i, v in enumerate(datos):
        ...
        y0 = ALTO - margen - h
        y1 = ALTO - margen
        color = "#50ff8a"                  #aqui se cambian los colores en hexadecimal, vsc me dejo cambiarlo por medio de una gui para elegir el color
...
datos = []
root = tk.Tk()
root.title("Visualizador - Selection Sort y Bubble Sort")
...
def ordenar_bubble(): #llama a la funcion para ordenar y controla la animacion
    if not datos: return
...
tk.Button(panel, text="Ordenar (Bubble)", command=ordenar_bubble).pack(side="left", padx=5) #boton bubble
